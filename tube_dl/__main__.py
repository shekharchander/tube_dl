...
class Youtube:
    def __init__(self, vid):
        ...
        try:
            vid = "".join(
                [i for i in findall(r"v=(.*?)&|youtu.be\/(.*?)&", vid + "&")[0]])
        except IndexError:
            vid = vid
        try:
    ...
    "cver": "2.20210114.08.00"
        ...
        try:
            y_data = get(
                f"https://youtube.com/watch?v={vid}&pbj=1", headers=headers).json()
        except Exception:
            from tube_dl.tdexceptions import VideoError
            raise VideoError(vid)
        yt_data = [i for i in y_data if "playerResponse" in i.keys()
                   ][0]["playerResponse"]
        ...
        extraDetails = [i for i in y_data if "response" in i.keys(
        )][0]["response"]["contents"]["twoColumnWatchNextResults"]["results"]["results"]["contents"]
        try:
            self.subscribers = extraDetails[1]['videoSecondaryInfoRenderer'][
                "owner"]["videoOwnerRenderer"]["subscriberCountText"]["runs"][0]
        except:
            self.subscribers = extraDetails[1]['videoSecondaryInfoRenderer']["owner"][
                "videoOwnerRenderer"]["subscriberCountText"]["simpleText"].replace(",", "")
        self.description = ''.join(
            [i["text"] for i in extraDetails[1]['videoSecondaryInfoRenderer']['description']["runs"]])
        extraDetails = extraDetails[0]["videoPrimaryInfoRenderer"]
        self.likes, self.dislikes = [i.strip(
        ) for i in extraDetails["sentimentBar"]["sentimentBarRenderer"]["tooltip"].split('/')]
        self.uploadDate = extraDetails["dateText"]["simpleText"].split(' ')[-1]
        try:
            self.hashTags = [
                i["text"] for i in extraDetails['superTitleLink']["runs"] if i["text"] != ' ']
        except Exception:
            self.hashTags = None
        try:
            mD = [i for i in y_data if "response" in i.keys()][0]["response"]["contents"]["twoColumnWatchNextResults"]["results"][
                "results"]["contents"][0]["videoSecondaryInfoRenderer"]["metadataRowContainer"]["metadataRowContainerRenderer"]["rows"]
            for i in mD:
                if "metadataRowRenderer" in i.keys():
                    self.meta[i["metadataRowRenderer"]["title"]["simpleText"]
                              ] = i["metadataRowRenderer"]["contents"][0]["simpleText"]
        except Exception:
            self.meta = dict()
        if self.isLive == 'true':
            try:
    ...
    def get_js(self):
        ...
        js_url = 'https://youtube.com/' + \
            findall(r'"jsUrl":"(.*?)"', base_data)[0]
    ...
    def Formats(self):
        ...
        for stream in self.streamingData:
            ...
            if 'signatureCipher' in stream.keys():
                signature, url = stream["signatureCipher"].split(
                    '&sp=sig&url=')
                signature = signature.replace('s=', '', 1).replace(
                    '%253D', '=').replace('%3D', '=')
            ...
            fmt.append(Format(self.category, description, self.title, {'itag': itag, 'mimeType': Mime, 'vcodec': vcodec, 'acodec': acodec,
                       'fps': fps, 'abr': abr, 'quality': quality, 'url': url, 'size': size, 'adaptive': adaptive, 'progressive': progressive}))
        return fmt
...
class Playlist:
    def __init__(self, url: str, start: int = None, end: int = None):
        ...
        try:
            ...
            total_count = int(
                findall(r'"stats":\[\{"runs":\[\{"text":"(.*?)"\}', html)[0].replace(',', ''))
        self.Api_key = findall(r'"innertubeApiKey":"(.*?)"', html)[0]
        self.version = findall(
            r'"INNERTUBE_CONTEXT_CLIENT_VERSION":"(.*?)"', html)[0]
        if total_count > 100:
    ...
    def fetch_continuation(self, html):
        ...
        data = post(url, json=payload, headers=headers).text.replace(
            ' ', '').replace('\n', '')
...
class Playlist:
    def __init__(self, url: str, start: int = None, end: int = None):
        ...
        if total_count > 100:
            start_html = html
            for _ in range(0, total_count//100):
                start_html = self.fetch_continuation(html)
        if start is None:
            start = 0
        if end is None:
            end = len(self.IDs)
